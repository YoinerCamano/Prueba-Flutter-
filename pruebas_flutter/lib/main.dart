...
void main() async {
  ...
  final BluetoothRepository bleRepo = BluetoothRepositoryBle(
      BleAdapter(config: BleUartConfig.truTest())); // Configuraci?n espec?fica para S3
        ...
        providers: [
          ...
          BlocProvider(
              create: (_) => DeviceInfoBloc(
                  bridgeRepo, commandRegistry, scaleProfileHolder, scaleModelRegistry)),
        ],
        child: MaterialApp(
...
class _BridgeRepository implements BluetoothRepository {
  ...
  BluetoothRepository _pick(String id) {
    ...
    print('?Y"? Seleccionando transporte ${transport.name} para ${descriptor.id} ($id)');
